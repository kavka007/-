...
# Генерация исходного ряда
rng = IrrationalNumberGenerator(lambda1, lambda2)
numbers = [rng.next() for _ in range(N)]

# Параметры исходного ряда
Mx = np.mean(numbers)
sigma_x_sq = np.var(numbers)
print(f"Начальное математическое ожидание Mx: {Mx}")
print(f"Начальная дисперсия sigma_x^2: {sigma_x_sq}")

# Центрирование
centered_numbers = np.array(numbers) - Mx

# Основная функция формирования z(k)
def process_z(centered_np, N, Ns, A1, A2, M0, sigma0_sq, sigma_x_sq, alpha0):
    coeff = math.sqrt(sigma0_sq / sigma_x_sq) * A1
    result = []
    for k in range(N - Ns):
        s = 0
        for i in range(k, k + Ns):
            s += centered_np[i] * math.exp(-A2 * alpha0 * (i - k))
        zk = coeff * s / Ns + M0
        result.append(zk)
...
def evaluate_params(x):
    ...
    z_test = process_z(centered_numbers, N, Ns, A1, A2, M0, sigma0_sq, sigma_x_sq, alpha0)
    Mz_test = np.mean(z_test)
    sigma_z_sq_test = np.var(z_test)

    Kz_test = [np.mean((z_test[:len(z_test) - S] - Mz_test) *
                       (z_test[S:] - Mz_test)) for S in range(20)]

    Kz_filtered = np.clip(np.abs(Kz_test[1:]), 1e-10, None)
    ln_Kz_test = np.log(Kz_filtered)
    coeffs_test = np.polyfit(np.arange(1, 20), ln_Kz_test, 1)
    alpha_z_test = -coeffs_test[0]

    err_M = abs(Mz_test - M0) / abs(M0)
    err_sigma = abs(sigma_z_sq_test - sigma0_sq) / abs(sigma0_sq)
    err_alpha = abs(alpha_z_test - alpha0) / abs(alpha0)

    return err_M + 3 * err_sigma + 10 * err_alpha
...
res = minimize(evaluate_params, [A1_init, A2_init],
...
# ----------------------------
z_opt = process_z(centered_numbers, N, Ns, A1_opt, A2_opt, M0, sigma0_sq, sigma_x_sq, alpha0)
Mz_opt = np.mean(z_opt)
sigma_z_sq_opt = np.var(z_opt)

Kz_opt = [np.mean((z_opt[:len(z_opt) - S] - Mz_opt) *
                  (z_opt[S:] - Mz_opt)) for S in range(20)]

ln_Kz_opt = np.log(np.clip(np.abs(Kz_opt[1:]), 1e-10, None))
coeffs_opt = np.polyfit(np.arange(1, 20), ln_Kz_opt, 1)
alpha_z_opt = -coeffs_opt[0]

# ----------------------------
# ВЫВОД РЕЗУЛЬТАТОВ
# ----------------------------
print(f"\nОптимизированные параметры: A1 = {A1_opt}, A2 = {A2_opt}")
...
# --- График 1 ---
axs[0].plot(range(N), numbers, label="Иррациональные числа",
...
# --- График 3 ---
axs[2].plot(range(20), Kz_opt, 'o-', markersize=6, linewidth=2,
            color='#2ca02c', label="Корреляционная функция Kz(S)")
axs[2].plot(range(20),
            sigma_z_sq_opt * np.exp(-alpha_z_opt * np.arange(20)),
